...
import org.jmouse.mapper.plugin.MappingPlugin;
...
public final class MappingConfig {
    ...
    private MappingConfig(Builder builder) {
        ...
        this.pluginBus                  = this.plugins.isEmpty()
                ? PluginBus.Noop.INSTANCE
                : new PluginBus(this.plugins);
    }
    ...
    /**
     * Return the dispatcher that feeds mapping lifecycle events to {@link #plugins()}.
     *
     * <p>The bus is built once with the configuration and shared by every mapping invocation, so
     * strategies may call this on the hot path. When no plugins are configured, a no-op bus is
     * returned.</p>
     *
     * @return shared plugin bus (never {@code null})
     */
    ...
    /**
     * How deep a single mapping may nest before it is refused.
     *
     * <p>The backstop under {@link ReferenceMappingPolicy}, and it answers for the graphs that policy
     * does not: a chain that is genuinely deep rather than circular, and a cycle closed through a shape
     * the reference policy does not track. Without it the failure is a bare {@link StackOverflowError}
     * raised somewhere inside the engine, naming neither the property nor the types involved.</p>
     *
    ...
    public static final class Builder {
        ...
        /**
         * Configure how deep one mapping may nest before it is refused.
         *
         * @param value max depth; must be {@code > 0}
         * @return this builder
         */
        public Builder maxDepth(int value) {
            Verify.state(value > 0, "maxDepth must be > 0");
            this.maxDepth = value;
            return this;
        }
