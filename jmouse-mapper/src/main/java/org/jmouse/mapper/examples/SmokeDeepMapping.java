...
public class SmokeDeepMapping {
    ...
    public static void main(String... arguments) {
        // Strategies change too, so this goes through the builder - but the rules are still added
        // to the defaults rather than replacing them. Every rename reads source -> target.
        Mapper mapper = Mappers.builder()
                .rules(rules -> rules
                        .mapping(AddressFlat.class, AddressView.class, address -> address
                                .rename("street", "line1")
                                .rename("zip", "postalCode")
                        )
                        .mapping(Map.class, AddressView.class, address -> address
                                .rename("street", "line1")
                                .rename("zip", "postalCode")
                        )
                        .mapping(AddressView.class, AddressFlat.class, address -> address
                                .rename("line1", "street")
                                .rename("postalCode", "zip")
                        )
                        .mapping(OrderLineSource.class, OrderLineTarget.class, line -> line
                                .rename("sku", "productCode")
                                .rename("quantity", "qty")
                                .rename("price", "lineAmount")
                        )
                        .mapping(OrderLineTarget.class, OrderLineSource.class, line -> line
                                .rename("productCode", "sku")
                                .rename("qty", "quantity")
                                .rename("lineAmount", "price")
                        )
                        .mapping(OrderSource.class, OrderTarget.class, order -> order
                                .rename("id", "orderId")
                                .rename("createdAt", "orderedAt")
                                .rename("buyer", "customer")
                                .rename("deliveryAddress", "shipping")
                                .rename("items", "lines")
                                .constant("status", "CREATED")
                        )
                        .mapping(OrderTarget.class, OrderSource.class, order -> order
                                .rename("orderId", "id")
                                .rename("orderedAt", "createdAt")
                                .rename("customer", "buyer")
                                .rename("shipping", "deliveryAddress")
                                .rename("lines", "items")
                        )
                )
                .strategyRegistry(
