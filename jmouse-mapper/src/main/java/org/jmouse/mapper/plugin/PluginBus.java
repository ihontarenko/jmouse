...
public class PluginBus {
     ...
     *
     * <p>⚠️ Ask this before <em>building</em> what a hook would be given, not only before calling one.
     * A {@link MappingValue} and a {@link org.jmouse.mapper.MappingDestination} are constructed
     * per property of every mapped object, and each of them needs the property path - which is parsed,
     * merged and cached to produce. With no plugins registered - the ordinary configuration - all of
     * that was built and immediately discarded, and the path alone was around a fifth of the engine's
     * time.</p>
     *
     * <p>An empty bus answers {@code false} whether it is a {@link Noop} or an ordinary one that was
     * given nothing, because the two are the same thing to a caller.</p>
     *
     * @return {@code true} when at least one plugin will see what is passed to this bus
     */
