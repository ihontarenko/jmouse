...
public final class PropertyMappingEvaluation implements PropertyMappingVisitor<Object> {
    ...
    /**
     * Guarded mapping evaluation.
     *
     * <p>⚠️ A false condition answers with {@link #IGNORED}, not {@code null}. That is the whole
     * difference from {@link PropertyMapping.When}: the property is left exactly as it was, and the
     * null policy is never consulted because there is no null to hand it.</p>
     *
     * @param mapping guarded mapping
     * @return the delegate result, or {@link #IGNORED}
     */
    @Override
    public Object visit(PropertyMapping.Guarded mapping) {
        return mapping.condition().test(source(), context)
                ? mapping.delegate().accept(this)
                : IGNORED;
    }
    ...
    /**
     * Expression mapping evaluation.
     *
     * <p>The source text is carried for whatever renders the mapping back; nothing here reads it.</p>
     *
     * @param mapping expression mapping
     * @return the computed value
     */
    @Override
    public Object visit(PropertyMapping.Expression mapping) {
        return mapping.function().compute(source(), context);
    }
