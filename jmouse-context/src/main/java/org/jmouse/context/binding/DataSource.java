...
public interface DataSource {
    ...
    default boolean isSimple() {
        return is(String.class) || is(Number.class) || is(Boolean.class) || is(Character.class);
    }
    ...
    default <K, V> Map<K, V> asMap(Class<K> keyType, Class<V> valueType) {
        ...
        ensureTypes("#asMap(Key, ?)", keyType, type.getFirst().getRawType());
        ensureTypes("#asMap(?, Value)", valueType, type.getLast().getRawType());
