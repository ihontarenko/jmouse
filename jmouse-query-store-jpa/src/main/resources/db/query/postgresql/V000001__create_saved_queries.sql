...
(
    ...
    --
    -- ⚠️ The library caps a body at 16000 characters, matching MySQL, where TEXT is sized in BYTES while
    -- everything above it counts characters. Without that gap a query written in Cyrillic
    -- would reach the limit at a quarter of the length an English one does. PostgreSQL TEXT is unbounded,
    -- but the cap is the library's and stays the same in both so a query cannot be savable in one only.
    --
