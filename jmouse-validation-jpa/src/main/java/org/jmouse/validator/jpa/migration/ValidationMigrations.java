 ...
 */
public final class ValidationMigrations {

    /** 📖 History table for this library's migrations. */
    public static final String HISTORY_TABLE = "validation_schema_history";

    /** 📂 Classpath root the per-dialect migration directories sit under. */
    public static final String LOCATION_ROOT = "db/validation";

    /** 🏷️ Bean name of the migrator, so a product's own Flyway can be ordered after it by name. */
    public static final String MIGRATOR_BEAN_NAME = "validationFlywayMigrator";

    private static final String LOCATION_PREFIX = "classpath:";
    private static final String SEPARATOR       = "/";

    private ValidationMigrations() {
    }
     ...
     * @return a Flyway location such as {@code classpath:db/validation/mysql}
     */
    public static String locationFor(ValidationDialect dialect) {
    ...
    public static String locationFor(DataSource dataSource) {
        return locationFor(ValidationDialect.resolve(dataSource));
    }
}
