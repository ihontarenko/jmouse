 ...
 * where the left operand is an {@link Character} and the right operand is any {@link Number},
 * as well as unary operations (INCREMENT, DECREMENT).
 * </p>
 */
public class CharacterOperationHandler implements OperationHandler<Character, Number> {
    ...
    public boolean supports(OperationType type, Class<?> x, Class<?> y) {
        boolean supported = x != null && Character.class.isAssignableFrom(x);
     ...
     * @param x         the left operand (non‐null {@link Character})
     * @param operand   the right operand (may be {@link Number} or {@code null} for unary ops)
     * @return the result as {@link Character} for integer ops or {@link Double} for exponentiation
     * @throws IllegalOperationException if division or modulus by zero is attempted
     */
    @Override
    public Object execute(OperationType type, Character x, Number operand) {
        char y = operand == null ? 0 : (char) operand.byteValue();
        ...
        return switch (type) {
            case PLUS        -> (char) (x + y);
            case MINUS       -> (char) (x - y);
            case MULTIPLY    -> (char) (x * y);
            case DIVIDE      -> (x / y);
            case MODULUS     -> (char) (x % y);
            case EXPONENTIAL -> Math.pow(x, y);
            case INCREMENT   -> (char) (x + 1);
            case DECREMENT   -> (char) (x - 1);
        };
    }
