...
/**
 * {@link OperationHandler} that performs byte‐based arithmetic.
 * <p>
 * Supports binary operations (PLUS, MINUS, MULTIPLY, DIVIDE, MODULUS, EXPONENTIAL)
 * where the left operand is an {@link Byte} and the right operand is any {@link Number},
 * as well as unary operations (INCREMENT, DECREMENT).
 * </p>
 */
public class ByteOperationHandler implements OperationHandler<Byte, Number> {
    ...
    public boolean supports(OperationType type, Class<?> x, Class<?> y) {
        boolean supported = x != null && Byte.class.isAssignableFrom(x);
     ...
     * @param x         the left operand (non‐null {@link Byte})
     * @param operand   the right operand (may be {@link Number} or {@code null} for unary ops)
     * @return the result as {@link Byte} for integer ops or {@link Double} for exponentiation
     * @throws IllegalOperationException if division or modulus by zero is attempted
     */
    @Override
    public Object execute(OperationType type, Byte x, Number operand) {
        byte y = (operand == null ? 0 : operand.byteValue());
        ...
        return switch (type) {
            case PLUS        -> (byte) (x + y);
            case MINUS       -> (byte) (x - y);
            case MULTIPLY    -> (byte) (x * y);
            case DIVIDE      -> (x / y);
            case MODULUS     -> (byte) (x % y);
            case EXPONENTIAL -> Math.pow(x, y);
            case INCREMENT   -> (byte) (x + 1);
            case DECREMENT   -> (byte) (x - 1);
        };
    }
