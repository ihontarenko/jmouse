...
import org.jmouse.web.match.PathContainer.Element;
...
/**
 * 🧭 Minimal route matched implementation that plugs into {@link PathPattern}.
 *
...
public class SimplePathPattern implements PathPattern {
    ...
    public String extractPath(String path) {
        final List<Element> elements   = PathPattern.split(path);
        ...
        return PathPattern.joinElements(elements, nonLiteral);
    }
    ...
    private RouteMatch doMatch(String path, boolean needExtracted) {
        ...
        while (i < parts.size()) {
            ...
            if (part instanceof PathSegment.CaptureAll captureAll) {
                String remainder = PathPattern.joinElements(elements, j);
        ...
        if (needExtracted && nonLiteralIndex >= 0 && elements.size() > nonLiteralIndex) {
            extracted = PathPattern.joinElements(elements, nonLiteralIndex);
        }
    ...
    sealed interface PathSegment permits
        ...
        final class CaptureOne implements PathSegment {
            ...
            public String toString() {
                return "CAPTURE_ONE : " + (constraint == null
                        ? "{%s}".formatted(name) : "{%s:%s}".formatted(name, constraint.pattern()));
            }
        ...
        final class CaptureAll implements PathSegment {
            ...
            public String toString() {
                return "CAPTURE_ALL : " + (name == null ? "**" : "{*%s}".formatted(name));
            }
